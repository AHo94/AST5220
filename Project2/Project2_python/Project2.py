...
class Redshift_mod():
	...
	def Get_Omegas_redshift(self, z):
		H = self.Get_Hubble_param_redshift(z)
		rho_c = rhoCrit_factor*H**2
		Omega_m_z = rho_m0*(1+z)**3/rho_c
		Omega_b_z = rho_b0*(1+z)**3/rho_c
		Omega_r_z = rho_r0*(1+z)**4/rho_c
		Omega_lambda_z = rho_lambda0/rho_c
	...
	def Get_n_b_redshift(self, z):
		Om_m, Om_b, Om_r, Om_lambda = self.Get_Omegas_redshift(z)
		H = self.Get_Hubble_param_redshift(z)
		rho_c = rhoCrit_factor*H**2
	...
	def Test_XE(self):
		...
		for i in range(0, self.n_eta-1):
			if X_e_array[i] > 0.99:
				X_e_array.append(self.Saha_eq_redshift(self.z_eta_rec[i]))
			else:
		...
		print len(self.z_eta_rec[:len(X_e_array2)])
		plt.semilogy(self.z_eta_rec[:len(X_e_array2)], X_e_array2)
		plt.show()
