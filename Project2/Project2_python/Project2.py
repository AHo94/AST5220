...
class time_mod():
	...
	def Plot_results(self, n_interp_points, x_start0 = -np.log(1.0 + 1630.4), x_end0 = -np.log(1.0 + 614.2)):
		...
		ax4 = plt.subplot(111)
		plt.hold("on")
		ax4.semilogy(self.x_eta, self.n_e, 'b-', label='Computed $n_e$')
		ax4.semilogy(x_eta_new, np.exp(n_e_NewLogarithmic), 'rx', label='Interpolated $n_e$')
		plt.xlabel('$x$')
		plt.ylabel('$n_e$')		
		...
		ax6 = plt.subplot(111)
		plt.hold("on")
		ax6.plot(self.x_eta, g_tilde, 'b-', label=r"$\tilde{g}$")
		ax6.plot(self.x_eta, g_tildeDerivative/10.0, 'r-', label=r"$\tilde{g}'/10$")
		ax6.plot(self.x_eta, g_tildeDoubleDer/300.0, 'g-', label=r"$\tilde{g}''/300$")
		ax6.set_xlim([-8,-6])
		plt.xlabel('$x$')
		plt.ylabel(r'$\tilde{g}$')
